    ...
    isset($_POST['password']) and $_POST['password'] !== '')
{
    ...
    if ($user)
    {
        if ( $password = password_verify($_POST['password'],$user->password))
        {
...
//    $password = password_verify($_POST['password']);
    ...
    {
        $errorPassword = "فیلد رمز عبور اجباری است";
    }
        ...
        <section style="height: 100vh; background-color: #138496;" class="d-flex justify-content-center align-items-center">
            <section style="width: 20rem;">
                ...
                <form class="pt-3 pb-1 px-2 bg-light rounded-bottom" action="" method="post">
                    <?php if (isset($error)){ ?>
                        <span class="text-danger">ایمیل یا رمز عبور اشتباه است</span>
                    <?php } ?>
